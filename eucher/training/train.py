...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train PyTorch ML models for Euchre")
    parser.add_argument(
        "--num-epochs",
        type=int,
        default=None,
    ...
    if data_path.exists():
        ...
        # Try loading from separate files and combine
        data_dir = config.training_data_dir
        action_types = ["order_up", "call_trump", "play_card", "discard"]
        
        for action_type in action_types:
            file_path = data_dir / f"training_{action_type}.json"
            if file_path.exists():
    ...
    if not examples:
        print(f"Training data not found at {data_path} or in {config.training_data_dir}")
