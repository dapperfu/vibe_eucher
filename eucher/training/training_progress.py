...
class TrainingProgressDisplay:
    ...
    def start_live_display(self, refresh_rate: float = 2.0) -> "Live":
        ...
        ----------
        refresh_rate : float
            Refresh rate in updates per second. Lower values reduce overhead.
            Default: 2.0 (was 4.0). Use 0.5-1.0 for profiling.
        ...
        if self.progress:
            ...
            return Live(layout, console=self.console, refresh_per_second=refresh_rate)
        else:
            # Just status table
            return Live(self.create_status_table(), console=self.console, refresh_per_second=refresh_rate)
