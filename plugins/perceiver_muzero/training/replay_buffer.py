...
class ReplayBuffer:
    ...
    def _sample_uniform(
        ...
        input_tokens_list = [s[0] for s in samples]
        actions = [s[1] for s in samples]
        policies = [s[2] for s in samples]
        values = [s[3] for s in samples]
        rewards = [s[4] for s in samples]
        next_input_tokens_list = [s[5] if s[5] is not None else s[0] for s in samples]
