...
class MSeriesBenchmarker:
    ...
    def run_training_progression_benchmark(self, training_configs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for i, config in enumerate(training_configs):
            ...
            stage_results = self._train_and_benchmark_stage(config, stage_name)
            progression_results[stage_name] = stage_results
            
            # Show improvement over previous stages
            if i > 0:
                prev_stage_name = f"stage_{i}"
                self._show_improvement(progression_results[prev_stage_name], stage_results)
