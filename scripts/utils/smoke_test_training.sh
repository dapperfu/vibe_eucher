...
if timeout 5 bash scripts/train_transformer_rl.sh --help > /dev/null 2>&1; then
...
if timeout 5 bash scripts/train_pytorch_ai.sh --help > /dev/null 2>&1; then
...
if timeout 5 python scripts/train_models.py --help > /dev/null 2>&1; then
...
if timeout 5 python scripts/train_models.py --mode train_gan 2>&1 | grep -q "Training data file not found\|FileNotFoundError"; then
