#!/usr/bin/env python3
"""
Train Classic AI Bots with Different Playing Profiles

This script trains the classic rule-based AI bots (Aggressive, Conservative, 
Balanced, Opportunistic) by running multiple games and analyzing their performance.
"""
...
def create_ai_players():
    """Create AI players with different playing profiles."""
    players = []
    
    # Create players with distinct personalities
    profiles = [
        ("Alice", "aggressive", 0.8),      # Very aggressive
        ("Bob", "conservative", 0.2),      # Very conservative  
        ("Charlie", "balanced", 0.5),      # Balanced
        ("David", "opportunistic", 0.7),   # Opportunistic
    ]
...
def run_training_games(num_games=1000, output_dir="training_results"):
    ...
    print(f"\n🎯 Starting training session with {num_games} games...")
    print("=" * 60)
...
def save_results(results, output_dir):
    ...
    results_file = Path(output_dir) / f"classic_ai_training_results_{timestamp}.json"
    
    with open(results_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Save summary
    summary_file = Path(output_dir) / f"classic_ai_training_summary_{timestamp}.txt"
    
    with open(summary_file, 'w') as f:
        f.write("CLASSIC AI TRAINING SESSION SUMMARY\n")
        f.write("=" * 50 + "\n\n")
...
def main():
    ...
    print("🧠 Classic AI Profile Training")
    print("=" * 40)
    print("This script will train the classic AI bots with different playing profiles:")
    print("  • Alice: Aggressive (risk ratio 0.8)")
    print("  • Bob: Conservative (risk ratio 0.2)")
    print("  • Charlie: Balanced (risk ratio 0.5)")
    print("  • David: Opportunistic (risk ratio 0.7)")
    print()
    
    # Configuration
    num_games = 1000  # Adjust this number as needed
