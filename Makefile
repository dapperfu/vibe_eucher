...
run: install ## Run the euchre game
	venv/bin/python -m euchre.cli.main play

ai-game: install ## Play AI vs AI game with full verbosity
	@echo "🤖 Starting AI vs AI Game with full verbosity..."
	venv/bin/python -m euchre.cli.main ai-vs-ai --very-verbose

ai-tournament: install ## Play AI tournament with configurable difficulty levels
	@echo "🏆 Starting AI Tournament..."
	@echo "Usage: make ai-tournament LEVEL=level2 STYLE=strategic RISK=0.7 MODEL_PATH=path/to/model.pth"
	@echo "Available levels: level1, level2, level3"
	@echo "Available styles: aggressive, conservative, balanced, opportunistic, strategic, intuitive"
	@echo "Example: make ai-tournament LEVEL=level2 STYLE=strategic RISK=0.7"
	venv/bin/python -m euchre.cli.main ai-tournament --ai-level $(or $(LEVEL),level1) --ai-style $(or $(STYLE),balanced) --risk-ratio $(or $(RISK),0.5) $(if $(MODEL_PATH),--model-path $(MODEL_PATH),)

ai-mix-match: install ## Play AI vs AI with mixed difficulty levels
	@echo "⚔️  Starting AI Mix Match..."
	@echo "Usage: make ai-mix-match TEAM1_LEVEL=level1 TEAM2_LEVEL=level3 TEAM1_STYLE=balanced TEAM2_STYLE=aggressive"
	@echo "Available levels: level1, level2, level3"
	@echo "Available styles: aggressive, conservative, balanced, opportunistic, strategic, intuitive"
	@echo "Example: make ai-mix-match TEAM1_LEVEL=level1 TEAM2_LEVEL=level3 TEAM1_STYLE=balanced TEAM2_STYLE=aggressive"
	venv/bin/python -m euchre.cli.main ai-mix-match --team1-level $(or $(TEAM1_LEVEL),level1) --team2-level $(or $(TEAM2_LEVEL),level1) --team1-style $(or $(TEAM1_STYLE),balanced) --team2-style $(or $(TEAM2_STYLE),balanced) --team1-risk $(or $(TEAM1_RISK),0.5) --team2-risk $(or $(TEAM2_RISK),0.5) $(if $(TEAM1_MODEL),--team1-model $(TEAM1_MODEL),) $(if $(TEAM2_MODEL),--team2-model $(TEAM2_MODEL),)

ai-info: install ## Display information about available AI types
	@echo "🤖 Showing AI Information..."
	venv/bin/python -m euchre.cli.main ai-info

ai-game-ncurses: install ## Play AI vs AI game with ncurses interface
	@echo "🖥️  Starting ncurses interface..."
	venv/bin/python -m euchre.cli.main ncurses
...
ncurses-human: install ## Run human vs AI euchre game with ncurses interface
	...
	venv/bin/python -m euchre.cli.main ncurses-human-vs-ai --position $(or $(POSITION),2)
...
logged: install ## Run AI vs AI euchre game with logging (no ncurses)
	venv/bin/python -m euchre.cli.main logged-game
...
profiles: install ## Run AI vs AI game with custom profiles and risk ratios
	venv/bin/python -m euchre.cli.main ai-profiles

mass-games: install ## Run thousands of games in parallel
	venv/bin/python -m euchre.cli.main run-mass-games
...
human-vs-ai: install ## Play human vs AI game with configurable AI types
	...
	venv/bin/python -m euchre.cli.main human-vs-ai $(PLAYER_NAME) --your-position $(POSITION) --partner-ai-type $(PARTNER_AI) --opponent1-ai-type $(OPP1_AI) --opponent2-ai-type $(OPP2_AI)
...
human-vs-level1: install ## Play human vs Level1 AI specifically
	...
	venv/bin/python -m euchre.cli.main human-vs-level1 $(PLAYER_NAME) --your-position $(POSITION) --level1-model $(MODEL)
...
human-vs-level2: install ## Play human vs Level2 AI specifically
	...
	venv/bin/python -m euchre.cli.main human-vs-level2 $(PLAYER_NAME) --your-position $(POSITION) --level2-model $(MODEL) --model-path $(MODEL_PATH)
...
train-self-play: install ## Train AI models using self-play
	venv/bin/python -m euchre.cli.main train-self-play --num-games 100000 --players Alice Bob Charlie David

train-integer-vs-float: install ## Train integer models against float models
	venv/bin/python -m euchre.cli.main train-integer-vs-float --num-games 200000

generate-profiles: install ## Generate 5 distinct AI player profiles with different playing styles
	venv/bin/python -m euchre.cli.main generate-player-profiles --num-games 15000 --output-dir trained_models --device auto --enable-amp

generate-profiles-cpu: install ## Generate AI player profiles using CPU only
	venv/bin/python -m euchre.cli.main generate-player-profiles --num-games 15000 --output-dir trained_models --device cpu

generate-profiles-gpu: install ## Generate AI player profiles with GPU acceleration
	venv/bin/python -m euchre.cli.main generate-player-profiles --num-games 15000 --output-dir trained_models --device cuda --enable-amp --gpu-memory-fraction 0.9

play-trained: install ## Play a game with trained AI players
	venv/bin/python -m euchre.cli.main play-trained-players --player1 Alice --player2 Bob --player3 Charlie --player4 David

tournament: install ## Run a tournament between trained players
	venv/bin/python -m euchre.cli.main tournament --num-games 100
...
neural-tournament: install ## Run neural network tournament (Alice vs Bob, 1000 games)
	venv/bin/python -m euchre.cli.main run-neural-games -m1 Alice -m2 Bob -n 1000
...
list-neural-models: install ## List available neural network models
	venv/bin/python -m euchre.cli.main list-neural-models

benchmark: install ## Run performance benchmark comparing float vs integer models
	venv/bin/python -m euchre.cli.main benchmark --device cpu --save-results
