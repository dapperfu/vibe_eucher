...
class TrainingProgressDisplay:
    ...
    def __init__(
        ...
        if show_progress_bar:
            ...
            # Start the progress bar so it can be displayed
            self.progress.start()
        else:
            self.progress = None
            self.task_id = None
    ...
    def update(self) -> None:
        ...
        if self.progress and self.task_id is not None:
            ...
            if scheduler_progress["mode"] == "time_based" and scheduler_progress["progress_percent"] is not None:
                # Set total to 100 for percentage display, update completed value
                ...
                # Convergence-based or no progress percentage - keep as indeterminate
                self.progress.update(
                    self.task_id,
                    description=f"Training (Game {status['current_game']})",
                )
    ...
    def print_summary(self) -> None:
        ...
        # Stop the progress bar before printing summary
        self.stop()
